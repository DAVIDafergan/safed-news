...
export const Home: React.FC = () => {
  ...
  return (
    <div className="animate-fade-in pb-20 bg-gray-50/30">
      ...
      {/* שטח פרסום חדש: עליון ביותר (מעל הסליידר) */}
      {topBannerAd && (
        <div className="bg-white border-b border-gray-100 py-2">
          <AdUnit ad={topBannerAd} className="container mx-auto h-20 md:h-24" />
        </div>
      )}
      ...
      <div className="flex flex-col lg:flex-row gap-10 lg:gap-16">
              ...
              {categoriesToShow.map((cat, index) => {
                 ...
                 return (
                   <React.Fragment key={cat}>
                     {/* הזרקת פרסומת בין הקטגוריה הראשונה לשניה */}
                     {index === 1 && categoryBetweenAd && (
                       <div className="mb-16">
                         <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 block text-center">בשיתוף פעולה</span>
                         <AdUnit ad={categoryBetweenAd} className="w-full rounded-xl shadow-sm border border-gray-100" />
                       </div>
                     )}
          ...
          <aside className="lg:w-1/3 flex flex-col gap-12">
            ...
            {/* פרסומת סיידבר תחתונה חדשה */}
            {sidebarBottomAd && (
               <div className="text-center">
                  <AdUnit ad={sidebarBottomAd} className="w-full rounded-2xl shadow-md border border-gray-100" />
               </div>
            )}
